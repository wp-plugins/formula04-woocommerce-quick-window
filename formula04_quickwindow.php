...
function compatible_plugin_running() {
	...
	case('woocommerce'):
	  ...
	  //wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/jquery.leanModal.min.js', array( 'jquery' ), $this->version, false );
		...
		$options = get_option( 'formula04_quickwindow_settings' );
...
function add_quick_window_button() {
	...
	echo do_shortcode('[formula04quickwindow]');
...
function add_quick_window_button_shortcode( $atts ) {
	...
	*/ 
	...
	?>
<a rel="leanModal" class="quick_button button <?php 

													echo $class ? $class : '';?>" <?php 

													echo $id ? 'id = "'.$id.'"' : '';

													?> href="#quick_window_wrapper" data-quick_window_id="<?php echo $product_id; ?>"> <?php echo $button_text; ?> </a>
<?php  //.quick_button
...
<?php //.woocommerce.quick-window.product ?>
<?php
		  endwhile;
		  wp_reset_postdata();
...
add_action( 'admin_init', 'formula04_quickwindow_settings_init' );
function formula04_quickwindow_settings_init(  ) { 
	...
	add_settings_section(
		...
		'formula04_quickwindow_settings_section_callback', 
		'F04QuickWindowSet'
	);
	...
	add_settings_section(
		...
		'formula04_quickwindow_button_settings_callback', 
		'F04QuickWindowSet'
	);		
	...
	add_settings_field( 
		...
		'formula04_quick_window_button_classes_render', 
		'F04QuickWindowSet', 
		'formula04_F04QuickWindowSet_quickbutton_section' 
	);	
	...
	add_settings_field( 
		...
		'formula04_quick_window_button_id_render', 
		'F04QuickWindowSet', 
		'formula04_F04QuickWindowSet_quickbutton_section' 
	);	
	...
	add_settings_section(
		...
		'formula04_quickwindow_settings_checkout_section_callback', 
		'F04QuickWindowSet'
	);
	...
	'formula04_quickwindow_allow_add_to_cart_render', 
	'F04QuickWindowSet', 
	'formula04_F04QuickWindowSet_checkout_section' 
	);

}
function formula04_quickwindow_allow_add_to_cart_render(  ) { 
	$options = get_option( 'formula04_quickwindow_settings' );?>
...
function formula04_quick_window_button_classes_render(  ) { 
	$options = get_option( 'formula04_quickwindow_settings' );
...
function formula04_quick_window_button_id_render(  ) { 
	$options = get_option( 'formula04_quickwindow_settings' );
...
function formula04_quickwindow_settings_checkout_section_callback( ){?>
<hr />

<?php }
function formula04_quickwindow_button_settings_callback(  ) { ?>
<hr />
<?php 
	//echo __( 'This section description', 'formula04' );
}
function formula04_quickwindow_settings_section_callback(  ) { ?>
<hr />
<?php 
	//echo __( 'This section description', 'formula04' );
}